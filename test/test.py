...
#f = open('/root/pose_evaluate_log.csv','w', newline='', encoding='utf-8')
#wr = csv.writer(f)
#wr.writerow(['image name', 'score'])
...
def test_net(tester, dets, det_range, gpu_id):
    ...
    # while img_start < 10:
        ...
        if vis and np.any(kps_result[:,:,2] > 0.9):
            tmpimg = cv2.imread(os.path.join(cfg.img_path, cropped_data[0]['imgpath']), cv2.IMREAD_IGNORE_ORIENTATION)
            tmpimg = tmpimg.astype('uint8')
        ...
        if cfg.dataset == 'COCO' or cfg.dataset == 'SWEETK' or cfg.dataset == 'K-Deep-Fashion':
            ...
            #wr.writerow([im_info['imgpath'], score_result])
            im_nm.append(im_info['imgpath'])
            score_list.append(score_result)
            print(" ", im_info['imgpath'], ": ", score_result)
        ...
        for i in range(len(kps_result)):
            if cfg.dataset == 'COCO':
                ...
                result = dict(image_id=im_info['image_id'], scores=score_result[i].round(4).tolist(),
                              keypoints=kps_result[i].round(3).tolist())
            elif cfg.dataset == 'K-Deep-Fashion':
                result = dict(image_id=im_info['image_id'], category_id=1, score=score_result[i].round(4).tolist(),
                              keypoints=kps_result[i].round(3).tolist())

            dump_results.append(result)

    return dump_results, im_nm, score_list
...
def test(test_model):
    ...
    # human bbox load
    if cfg.useGTbbox and cfg.testset in ['train', 'val', 'test']:
        if cfg.testset == 'train':
            dets = d.load_train_data(score=True)
        elif cfg.testset == 'val':
            dets = d.load_val_data_with_annot()
        else:
            # with open('/root/nia_pose_estimation/data/K-Deep-Fashion/annotations/test.json', 'r') as f:
            #     dets = json.load(f)
            dets = d.load_test_data_with_annot()
    ...
    print('length of im_nm', len(result[1]))
    print('length of score_list', len(result[2]))
    log = pd.DataFrame({'img_name': result[1], 'score': result[2]})
    log.to_csv('/root/pose_evaluate_log.csv', index=False, encoding='utf-8')

    # evaluation
    d.evaluation(result[0], annot, cfg.result_dir, cfg.testset)
...
if __name__ == '__main__':
    ...
    #generate csv file
    # print('length of im_nm', len(im_nm))
    # print('length of score_list', len(score_list))
    # log = pd.DataFrame({'img_name': im_nm, 'score': score_list})
    # log.to_csv('/root/pose_evaluate_log.csv', index=False, encoding='utf-8')
