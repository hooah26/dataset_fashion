...
import networks
from utils.miou import compute_mean_ioU_sweetk_once
...
SWEETK_SHAPELESS = ["Background","Face","Left-arm","Right-arm","Left-leg", "Right-leg","Normal_top","Normal_bottom","Coat","jacket","Jumper","Padding","vest","Cardigan","Blouse","Top","T-shirt","shirts","Sweater","Pants","Skirt","Dress","jumpsuit"]
...
def get_arguments():
    ...
    parser.add_argument("--save-results", action="store_false", help="whether to save the results.")
    ...
    # Shapeless
    # parser.add_argument("--data-dir", type=str, default='/workspace/data/dataset/20221101/K-Deep-Fashion-Shapeless/쉐이프리스 의류 및 포즈 데이터/')
...
def multi_scale_testing(model, batch_input_im, crop_size=[473, 473],  multi_scales=[1]):
    ...
    ms_fused_parsing_output = ms_fused_parsing_output#.mean(1)#.mean(0)
    ms_fused_parsing_output = ms_fused_parsing_output.permute(0,2,3,1)#.permute(1, 2, 0)  # HWC
    parsing = torch.argmax( ms_fused_parsing_output,dim=3)#, dim=2)
