...
from utils.miou import compute_mean_ioU_sweetk_once, SegmentationMetrics
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(testloader)):
            ...
            for index, p in enumerate(parsing) :
                ...
                gt = parsing_preds[0][1]
                pred = torch.Tensor(parsing_preds[0][0])
                pred = pred.unsqueeze(0)
                seg_metrics = SegmentationMetrics()
                _, _, precision, _ = seg_metrics._calculate_multi_metrics(gt, pred, args.num_classes)
                print(precision)
