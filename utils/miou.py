...
def compute_mean_ioU_sweetk(val_file, preds, scales, centers, num_classes,
                            datadir,LABELS, input_size=[473, 473], dataset='val' ):
    # val_file = os.path.join(datadir, dataset + '_id.txt')
    val_id = val_file #[i_id.strip() for i_id in open(val_file)]

    confusion_matrix = np.zeros((num_classes, num_classes))

    for i, (pred_out, gt) in enumerate(preds):
        im_name = val_id[i]
        # gt_path = os.path.join(datadir, dataset + '_segmentations', im_name + '.png')
        # gt = np.array(PILImage.open(gt_path))
        ...
        gt = np.asarray(gt[0], dtype=np.int32)
        pred = np.asarray(pred, dtype=np.int32)
...
def compute_mean_ioU_sweetk_once(val_file, preds, scales, centers, num_classes,
                            datadir,LABELS, input_size=[473, 473], dataset='val' ):
    confusion_matrix = np.zeros((num_classes, num_classes))
    for i, (pred_out, gt) in enumerate(preds):
        _, h, w = gt.shape
    ...
    for i, (label, iou) in enumerate(zip(LABELS, IoU_array)):
        ...
        if iou != 0 :
            cnt+=1
    mean_accuracy = (np.sum(tp / np.maximum(1.0, pos))/int(cnt)) * 100
    # mean_accuracy = ((tp / np.maximum(1.0, pos)).mean()) * 100
    mean_IoU = round(int(miou)/int(cnt), 2)
    name_value.append(('Pixel accuracy', pixel_accuracy))
    name_value.append(('Mean accuracy', mean_accuracy))
    name_value.append(('Mean IU', mean_IoU))
    name_value = OrderedDict(name_value)
